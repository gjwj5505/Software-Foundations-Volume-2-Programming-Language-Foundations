...
Theorem hoare_skip : forall P,
     {{P}} skip {{P}}.
Proof.
  intros P st st' Hc HP. inversion Hc. subst. easy.
Qed.
...
Proof.
  ...
  intros P Q R c1 c2 Hc2 Hc1 st st' Hstc1c2 HstP.
  inversion Hstc1c2. 
  (* 여기서 eauto 가능 *)
  subst. clear Hstc1c2.
  apply Hc1 in H1. apply Hc2 in H4. auto.
Qed.
...
Example equivalent_assertion1 :
  {{ (X <= 5) [X |-> 3] }} <<->> {{ 3 <= 5 }}.
Proof.
  split; unfold assert_implies, assertion_sub; eauto.
Qed.
...
Theorem hoare_asgn : forall Q X (a:aexp),
  {{Q [X |-> a]}} X := a {{Q}}.
Proof.
  intros Q X a st st' Hc HQ.
  unfold assertion_sub in HQ.
  inversion Hc. subst. clear Hc. simpl in HQ. eauto.
Qed.
...
Proof.
  ...
  specialize contra with (st := empty_st) (st' := (X !-> 1)).
  assert (empty_st =[ X := X + 1 ]=> (X !-> 1)).
  {
    apply E_Asgn. reflexivity.
  }
  apply contra in H.
  - clear contra. simpl in H. rewrite t_update_eq in H. discriminate.
  - easy.
Qed.
(* FILL IN HERE
...
Theorem hoare_asgn_fwd :
  ...
  {{ $(fun st => (P (X !-> m ; st)
             /\ st X = aeval (X !-> m ; st) a))  }}.
Proof.
  intros m a P st st' Hasgn [HP HXm].
  split.
  - inversion Hasgn. subst. clear Hasgn.
    rewrite t_update_shadow.
    replace m with (Aexp_of_nat m st).
    + rewrite <- HXm.
  ...
  - inversion Hasgn. subst. clear Hasgn.
    rewrite t_update_shadow.
    rewrite t_update_eq.
    replace (X !-> m; st) with st.
    + easy.
    + replace m with (Aexp_of_nat m st).
      -- rewrite <- HXm. rewrite t_update_same. easy.
      -- easy.
Qed.
...
Theorem hoare_asgn_fwd_exists :
  ...
  {{ $(fun st => exists m, P (X !-> m ; st) /\
                st X = aeval (X !-> m ; st) a) }}.
Proof.
  intros a P st st' Hasgn HP.
  exists (st X).
  specialize hoare_asgn_fwd with (m := st X) (a := a) (P := P) as hoare_asgn_fwd'.
  unfold valid_hoare_triple in hoare_asgn_fwd'.
  specialize hoare_asgn_fwd' with (st := st) (st' := st').
...
Proof.
  ...
  - eapply hoare_asgn.
  - unfold "->>", assertion_sub, t_update. intros st _. eauto.
...
Theorem swap_exercise :
  ...
  {{Y <= X}}.
Proof.
  eapply hoare_seq with (Q := {{X - Y <= Y}}).
  - eapply hoare_seq with (Q := {{Y <= X - Y}}).
    + eapply hoare_consequence_pre.
      -- eapply hoare_asgn.
      -- assertion_auto.
    + eapply hoare_consequence_pre.
...
Proof.
  unfold valid_hoare_triple.
  intros contra.
  specialize contra with (a := <{AId X}>) (n := 0) (st := empty_st) (st' := (Y !-> 3 ; X !-> 3)).
  assert (H1 : empty_st =[ X := 3; Y := X ]=> (Y !-> 3; X !-> 3)).
  {
    clear contra.
    eapply E_Seq.
    - eapply E_Asgn. eauto. 
  ...
  assert (H2 : Aexp_of_aexp <{ Y + 1 }> empty_st = Aexp_of_aexp 1 empty_st).
  {
    clear contra.
    reflexivity.
  } 
  assert (H3 : ~ (Aexp_of_aexp  Y (Y !-> 3; X !-> 3) = Aexp_of_aexp O (Y !-> 3; X !-> 3))).
  {
    clear contra.
    simpl. 
    rewrite t_update_eq.
...
Example if_example :
(* assertion_auto, assertion_auto', assertion_auto''가 뭐임 데체 ㅋㅋ *)
  {{True}}
    if (X = 0)
      then Y := 2
      else Y := X + 1
...
Proof.
  ...
  - 
    ...
    Unshelve.
    + apply O.
    + apply S. apply S. apply S. apply S. apply O.  
...
Proof.
  ...
  remember <{repeat c until b end}> as rcmd eqn:Heq.
  intros st st'' Hrep HP.
  induction Hrep; inversion Heq; subst; clear Heq.
  - (*repeat false*)
    assert (repeat_end : forall st st', 
    st =[ repeat c until b end ]=> st' ->
    Q st -> 
    beval st b = false -> 
    Q st' /\ beval st' b = true).
    {
      intros ST ST' HrepST HQST HbSTF.
      remember <{repeat c until b end}> as original_command eqn:Horig.
      induction HrepST; inversion Horig; subst; clear Horig; eauto.
    }
    eapply repeat_end.
    + eapply Hrep2.
    + eauto.
...
Proof.
  intros.
  intros st st' Hhavoc Hpre.
  inversion Hhavoc. subst. clear Hhavoc.
  unfold havoc_pre in Hpre. apply Hpre.
Qed.
(** [] *)
...
Proof.
  ...
  exists True.
  split.
  - intros st res Hassume HP.
    exists (X !-> 1; st).
    inversion Hassume. subst. clear Hassume.
    split.
    + f_equal. replace (1) with (st X).
      -- rewrite t_update_same. easy.
      -- simpl in H0. apply Nat.eqb_eq in H0. eauto.
    + simpl. easy.
  - intros contra.
    unfold valid_hoare_triple in contra.
    specialize contra with (st := (X !-> 2)) (r := RError).
    assert ((X !-> 2) =[ assert (X = 1) ]=> RError).
    {
      apply E_AssertFalse.
      simpl. easy. 
    }
...
Theorem assert_implies_assume : forall P b Q,
  ...
  -> ({{P}} assume b {{Q}}).
Proof.
  intros P b Q Hassert st r Hassume HP.
  exists st.
  unfold valid_hoare_triple in Hassert.
  specialize Hassert with (st := st) (r := r).
  inversion Hassume; subst; clear Hassume.
