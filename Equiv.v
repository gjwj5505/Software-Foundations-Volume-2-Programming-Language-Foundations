...
Proof.
  ...
  split; intros H.
  - inversion H.
    + unfold bequiv in Hbf. simpl in Hbf. specialize Hbf with (st := st). rewrite Hbf in H5. discriminate.
    + assumption.
  - apply E_IfFalse.
    + apply Hbf. 
    + assumption.
...
Proof.
  intros b c1 c2. unfold cequiv. intros st st'.
  ...
  - eapply E_IfTrue.
    ...
    + assumption.
  - eapply E_IfFalse.
    + simpl in H5. destruct (beval st b) eqn:Db.
      -- discriminate H5.
      -- reflexivity.
    + assumption.
  - eapply E_IfTrue.
...
Proof.
  ...
  induction H;
  inversion Heqcwhile; subst; clear Heqcwhile.
  - unfold bequiv in Hb. rewrite Hb in H. simpl in H. discriminate.
...
Proof.
  ...
  - Search "E_Asgn".
    assert (Asgn_eq : st' = (x !-> st' x; st')). 
    {
      rewrite t_update_same.
      reflexivity. 
    }
    rewrite Asgn_eq at 2.
...
Proof.
  ...
  intros st st'.
  split; intros H; inversion H; subst; clear H.
  - replace (st') with (X !-> aeval st' a ; st') at 2.
    + apply E_Asgn. reflexivity.
    + unfold aequiv in Hxa. rewrite <- Hxa.
      rewrite t_update_same. reflexivity.
  - replace (X !-> aeval st a; st) with (st).
    + apply E_Skip.
    + unfold aequiv in Hxa. rewrite <- Hxa. simpl. rewrite t_update_same. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  {
    intros c1 c1' c2 c2' st st' Hc1 Hc2 Hc.
...
Proof.
  ...
  - (* while *)
    destruct (fold_constants_bexp b) eqn:Dfcbb;
    try (
      ...
      try ( 
        ...
        try (
          rewrite Dfcbb; apply refl_bequiv
        )
      );
      assumption
    ).
    + apply while_true.
      apply trans_bequiv with (b2 := (fold_constants_bexp b)).
      -- apply fold_constants_bexp_sound.
      -- rewrite Dfcbb. apply refl_bequiv.
    + apply while_false.
      eapply trans_bequiv.
...
Proof.
  ...
  induction Hvnu; subst;
  try reflexivity;
  try (simpl; rewrite IHHvnu1; rewrite IHHvnu2; simpl; reflexivity).
...
Proof.
  ...
  specialize contra with (st := empty_st) (st' := empty_st).
  destruct contra as [_ contra'].
  assert (empty_st =[ skip ]=> empty_st) by (apply E_Skip).
  apply contra' in H. clear contra'.

  remember <{while true do skip end}> as cwhile.
  induction H; try auto; try discriminate.
...
Proof.
  ...
  ).
  {
    ...
    inversion H. subst. clear H.
      ...
      destruct (X =? Y)%string eqn:XeqbY.
      - apply String.eqb_eq in XeqbY. rewrite XeqbY. 
        rewrite t_update_shadow.
        eapply E_Seq.
        + eapply E_Havoc.
        + rewrite XeqbY.
          erewrite <- t_update_shadow at 2.
          eapply E_Havoc. 
...
Proof.
  right. intros contra.
  unfold cequiv in contra.
  specialize contra with (st := empty_st) (st' := (Y !-> 2 ; X !-> 1)).
  destruct contra as [contra' _].
  assert (empty_st =[ ptwice ]=> (Y !-> 2; X !-> 1)).
  {
    clear contra'.
    eapply E_Seq.
    - eapply E_Havoc.
  ...
  assert (~ (empty_st =[ pcopy ]=> (Y !-> 2; X !-> 1))).
  {
    clear contra'. clear H.
    intros contra.
    inversion contra. subst. clear contra.
    clear H1.
    inversion H4. subst. clear H4.
    simpl in H3.
    destruct ((Y !-> st' X; st') X =? 1) eqn:DX.
    - rewrite t_update_neq in DX.
      + rewrite eqb_eq in DX. rewrite DX in H3.
        assert ((Y !-> 1; st') Y = (Y !-> 2; X !-> 1) Y).
        {
          rewrite H3. reflexivity. 
        }
        rewrite !t_update_eq in H. discriminate H.
      + intros contra. discriminate contra.
    - rewrite H3 in DX. clear H3.
      rewrite t_update_neq in DX.
      + rewrite t_update_eq in DX. discriminate DX.
      + intros contra. discriminate contra.
  }
  apply contra' in H. apply H0 in H. assumption.
Qed.
(** [] *)
...
Proof.
  ...
  remember p1 as p1' eqn:Rp1.
  induction contra; subst; try reflexivity; try discriminate.
  - inversion Rp1. subst. clear Rp1.
    unfold beval in H0. destruct (aeval st X =? aeval st 0) eqn:DX.
    + apply eqb_eq in DX. simpl in DX. apply H in DX. assumption.
    + discriminate H0.
  - inversion contra1; subst; try discriminate; clear contra1.
    inversion H2; subst; try discriminate; clear H2.
    assert (ASSERT : (x !-> aeval st'0 a; st'0) X <> 0).
    {
      inversion Rp1. subst.
      rewrite t_update_eq. unfold aeval.
      inversion H1. subst. rewrite t_update_neq.
      - lia.
      - intros contra. discriminate contra. 
    }
    apply IHcontra2 in ASSERT.
    + assumption.
    + assumption.
Qed.
...
Proof.
  intros. intros contra.
  remember p2 as p2' eqn:Rp2.
  induction contra; subst; try reflexivity; try discriminate.
  - inversion Rp2. subst. clear Rp2.
    unfold beval in H0. destruct (aeval st X =? aeval st 0) eqn:DX.
    + apply eqb_eq in DX. simpl in DX. apply H in DX. assumption.
    + discriminate H0.
  - inversion contra1; subst; try discriminate; clear contra1.
    apply IHcontra2 in H.
    + assumption.
    + assumption.
Qed.
...
Theorem p1_p2_equiv : cequiv p1 p2.
  intros.
  split; intros H; destruct (st X) eqn:DstX; inversion H; try discriminate; subst;
  try apply E_WhileFalse; try assumption; try discriminate;
  try apply p1_may_diverge in H; try easy.
  - unfold beval in H2. unfold aeval in H2. intros contra. rewrite contra in H2. discriminate H2.
  - rewrite DstX. lia.
  - inversion H; subst; clear H.
    + apply E_WhileFalse. unfold beval. unfold aeval. rewrite DstX. reflexivity.
    + unfold beval in H2. unfold aeval in H2. rewrite DstX in H2. discriminate H2.
  - inversion H6; subst; clear H6.
    + inversion H; subst; clear H.
      -- apply E_WhileFalse. assumption.
      -- unfold beval in *. unfold aeval in *. inversion H3. subst. rewrite H2 in H7. discriminate.
    + inversion H; subst; clear H.
      -- apply E_WhileFalse. assumption.
      -- inversion H3; inversion H5; subst; clear H3; clear H5.
         inversion H7; subst; clear H7.
         apply p2_may_diverge in H11.
         ++ easy.
         ++ intros contra. rewrite DstX in contra. discriminate.
Qed.
...
Proof.
  intros contra.
  unfold cequiv in contra.
  specialize contra with (st := (X !-> 1)) (st' := (X !-> 0; Z !-> 0)).
  destruct contra as [contra' _].
  assert ((X !-> 1) =[ p3 ]=> (X !-> 0; Z !-> 0)).
  {
    clear contra'.
    eapply E_Seq.
    - eapply E_Asgn. simpl. reflexivity.
    - eapply E_WhileTrue.
      + unfold beval. simpl. reflexivity.
      + eapply E_Seq.
        -- eapply E_Havoc with (n := 0).
        -- eapply E_Havoc with (n := 0).
      + replace (Z !-> 0; X !-> 0; Z !-> 1; X !-> 1) with (X !-> 0; Z !-> 0).
        -- eapply E_WhileFalse. simpl. reflexivity.
        -- apply functional_extensionality.
           intros x.
           destruct (x =? X)%string eqn:DxX;
           destruct (x =? Z)%string eqn:DxZ;
           try rewrite String.eqb_eq in DxX;
           try rewrite String.eqb_neq in DxX;
           try rewrite String.eqb_eq in DxZ;
           try rewrite String.eqb_neq in DxZ; subst.
           ++ rewrite DxZ. reflexivity.
           ...
           ++ rewrite t_update_eq. rewrite t_update_neq.
            --- easy.
            --- easy.
           ++ rewrite !t_update_neq; try assumption; try easy.
  }
  apply contra' in H.
  inversion H; subst; clear H.
  inversion H2; subst; clear H2.
  inversion H5; subst; clear H5.
  simpl in H3.
  assert ((Z !-> 1; X !-> 0; X !-> 1) Z = (X !-> 0; Z !-> 0) Z).
  {
    rewrite H3. reflexivity. 
  }
  simpl in H.
  replace ((X !-> 0; Z !-> 0) Z) with ((Z !-> 0) Z) in H.
...
Proof.
  ...
  split.
  - intros Hwhile.
    remember p5 as p5' eqn:Rp5.
    induction Hwhile; try discriminate; try auto.
    + inversion Rp5; subst; clear Rp5.
      unfold beval in H. unfold aeval in H. rewrite negb_false_iff in H. apply eqb_eq in H.
      replace st with (X !-> 1 ;st) at 2.
      -- eapply E_Asgn. unfold aeval. reflexivity.
      -- apply functional_extensionality.
         intros x.
         destruct ((x =? X)%string) eqn:DxX.
         ++ apply String.eqb_eq in DxX. subst. rewrite H. apply t_update_eq.
         ++ apply String.eqb_neq in DxX. apply t_update_neq. easy.
    + apply IHHwhile2 in Rp5 as Hst'st''.
      destruct p6 eqn:Dp6; try discriminate.
      remember s as X. clear HeqX. clear s. clear IHHwhile1. clear IHHwhile2.
      inversion Dp6. rewrite H1. clear H1.
      assert (forall x, x <> X -> st x = st' x).
      {
        inversion Hwhile1; subst; try discriminate.
        inversion Rp5. subst.
        intros x. intros xneqX.
      ...
      {
        intros x xneqX. 
        inversion Hst'st''. subst.
        rewrite t_update_neq.
        - easy.
        - easy.
      }
      assert (rep: st'' = (X !-> 1 ; st)).
      {
        inversion Hst'st''. rewrite <- H2 in *. unfold aeval in H7. rewrite H7. 
        inversion Hwhile1; subst; try discriminate; try reflexivity.
        apply functional_extensionality. intros x.
        inversion Rp5. subst.
        destruct (x =? X)%string eqn:DxX.
        - apply String.eqb_eq in DxX. subst. rewrite !t_update_eq. reflexivity.
        - rewrite String.eqb_neq in DxX. rewrite t_update_neq.
          + rewrite <- H0.
            -- rewrite t_update_neq.
              ++ reflexivity.
              ++ easy.
      ...
      easy.
  - intros Hp6.
    remember p6 as p6' eqn:Rp6.
    inversion Hp6; subst; try discriminate; try reflexivity.
    inversion H0. subst. clear H0.
    destruct (st X =? 1) eqn:DstX1.
    + rewrite eqb_eq in DstX1.
      assert (rep : (X !-> aeval st 1; st) = st).
      {
        unfold aeval.
        replace 1 with (st X).
        apply t_update_same.
      ...
      rewrite rep. clear rep.
      eapply E_WhileFalse.
      simpl. rewrite DstX1. reflexivity.
    + rewrite eqb_neq in DstX1.
      eapply E_WhileTrue.
      -- simpl. rewrite <- eqb_neq in DstX1. rewrite DstX1. reflexivity.
      -- eapply E_Havoc.
      -- eapply E_WhileFalse.
         simpl. reflexivity.
Qed.
